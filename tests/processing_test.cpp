...
TEST(ProcessingTest, PreprocessImage)
{
    ROS_INFO("Open image");
...
TEST(ProcessingTest, LeastSquaresFit)
{
    ...
    processing::Resolution resolution(320, 176);
    processing::Curve curve = processing::calculateCurve(coefficients, resolution, 9);

    ASSERT_EQ(curve.points.size(), 9);
    ASSERT_GE(curve.y_min, 0);
    ASSERT_LE(curve.y_max, resolution.height);
}

TEST(ProcessingTest, RansacFit)
{
    ...
    ROS_INFO("Calculate curve");
    processing::Resolution resolution(320, 176);
    processing::Curve curve = processing::calculateCurve(coefficients, resolution, 15);

    ASSERT_EQ(curve.points.size(), 15);
    ASSERT_GE(curve.y_min, 0);
    ASSERT_LE(curve.y_max, resolution.height);
}